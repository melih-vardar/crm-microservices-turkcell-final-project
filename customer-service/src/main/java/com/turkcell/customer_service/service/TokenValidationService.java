...
public class TokenValidationService {
    ...
    public UUID getUserIdFromToken(String token) {
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null
                ...
                log.info("Successfully extracted user ID: {}", response.getBody().getUserId());
                return response.getBody().getUserId();
            }

            log.warn("Failed to extract user ID, token validation failed");
            return null;
        } catch (Exception e) {
    ...
    public String getUsernameFromToken(String token) {
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null
                    && response.getBody().isValid()) {
                log.info("Successfully extracted username: {}", response.getBody().getUsername());
                return response.getBody().getUsername();
            }

            log.warn("Failed to extract username, token validation failed");
            return null;
        } catch (Exception e) {
